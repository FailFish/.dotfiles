...
-- `<C-n>` / `<C-p>` to scroll completion options.
-- `<C-f>` / `<C-b>` to scroll preview.
-- `<C-e>` to discard / `<C-y>` to select.
...
later(function()
  ...
  ai.setup({
    ...
    custom_textobjects = {
      ...
      -- Ported from old config's 'mini.ai' setup in 'lua/user/plugins/editing.lua'.
